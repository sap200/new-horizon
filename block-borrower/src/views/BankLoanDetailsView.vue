  ...
  <div class="ln-dt-vw">
      ...
      <div id="rps-tbl">
          <table v-if="this.rpsLoaded">
              ...
              <tbody>
                  <tr v-for="item in rps" :key="item.rpsIndex"> 
                      <td> {{item.rpsIndex}} </td>
...
export default {
  ...
  methods: {
    ...
    async calculateDues() {
        const lid = this.$route.params.loanId

        const web3i = new web3(EVM_SIDECHAIN_RPC);
        try {
            const contract = new web3i.eth.Contract(blockBorrowerAbi, BLOCKBORROWER_CONTRACT_ADDRESS);
            // eslint-disable-next-line
            const dues = await contract.methods.calculateDues(BigInt(lid)).call({})

            this.numDues = Number(dues);
        } catch (error) {
            console.log(error);
            return null;
        }
    ...
    async startAuction() {
            ...
            try {
                ...
                let txnParams = {
                    to: BLOCKBORROWER_CONTRACT_ADDRESS,
                    from: sender,
                    data: window.contract.methods.startAuction(bankId, loanId).encodeABI(),
                }
                ...
                console.log('Transaction hash:', txnHash)
                setTimeout(() => {
                    this.getTxnReceiptAction(txnHash, "Auction Started for this Loan !!")
                }, 5000);
    ...
    async finalizeAuction() {
            ...
            try {
                ...
                let txnParams = {
                    to: BLOCKBORROWER_CONTRACT_ADDRESS,
                    from: sender,
                    data: window.contract.methods.finalizeAuctionBeforeTime(bankId, loanId).encodeABI(),
                }
                ...
                console.log('Transaction hash:', txnHash)
                setTimeout(() => {
                    this.getTxnReceiptAction(txnHash, "Auction Finalized for this Loan !!")
                }, 5000);
    ...
    async cancelAuction() {
            ...
            try {
                ...
                let txnParams = {
                    to: BLOCKBORROWER_CONTRACT_ADDRESS,
                    from: sender,
                    data: window.contract.methods.cancelAuctionBeforeTime(bankId, loanId).encodeABI(),
                }
                ...
                console.log('Transaction hash:', txnHash)
                setTimeout(() => {
                    this.getTxnReceiptAction(txnHash, "Auction Cancelled for this Loan !!")
                }, 5000);
